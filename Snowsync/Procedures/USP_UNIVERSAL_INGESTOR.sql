-- ============================================================================
-- USP_UNIVERSAL_INGESTOR — Enhanced Production Procedure
-- Supports: GET/POST, CURSOR/LINK/PAGE/OFFSET pagination, configurable
-- page size, records path, dynamic filters, POST body, run summary logging,
-- Retry-After for 429, fast-fail on non-retryable codes.
-- ============================================================================
-- NOTE: This is the TEMPLATE. USP_REBUILD_INGESTOR dynamically recreates this
-- procedure with the correct SECRETS clause bound to your active configs.
-- Run USP_REBUILD_INGESTOR() after deploying this file.
-- ============================================================================
...
def authenticate(auth_type, headers, cfg_dict):
    ...
    if auth_type == "API_KEY":
        ...
        raise ValueError(f"Unknown AUTH_TYPE '{auth_type}'. Valid: NONE, API_KEY, OAUTH2_BASIC, OAUTH2_INTEGRATION")
...
def main(session, api_target):
    ...
    if pagination_type == "CURSOR" and not cursor_path:
        err = (f"Error: PAGINATION_TYPE=CURSOR but CURSOR_PATH is not configured for '{api_target}'. "
               f"Set CURSOR_PATH to the dotted JSON path where the next cursor lives.")
        log_run_summary(session, run_id, api_target, api_id, run_start, 0, 0, 0, 0, "ERROR",
    ...
    if pagination_type == "CURSOR" and not cursor_param:
        err = (f"Error: PAGINATION_TYPE=CURSOR but CURSOR_PARAM is not configured for '{api_target}'. "
               f"Set CURSOR_PARAM to the query parameter name (e.g. 'after', 'starting_after').")
        log_run_summary(session, run_id, api_target, api_id, run_start, 0, 0, 0, 0, "ERROR",
                        last_sync_value, None, pagination_type, err)
        return err
    ...
    while True:
        if pages_processed >= max_pages:
            msg = f"Warning: '{api_target}' reached MAX_PAGES limit ({max_pages}). {pages_processed} page(s) ingested."
        ...
        if not response or status != 200:
            ...
            err_msg = (f"Error: '{api_target}' failed on page {display_page}. "
                       f"URL: {url} | Status: {status} | Pages before failure: {pages_processed}")
            log_run_summary(session, run_id, api_target, api_id, run_start, pages_processed, total_chunks_all,
        ...
        try:
            ...
            err_msg = f"Error: '{api_target}' returned non-JSON body. {str(json_err)}"
            log_run_summary(session, run_id, api_target, api_id, run_start, pages_processed, total_chunks_all,
                            records_ingested, records_skipped, "ERROR",
        ...
        if data_to_split:
            ...
            for i in range(0, len(data_to_split), CHUNK_SIZE):
                ...
                chunk_payload = {
                    "data": chunk_data,
                    "_chunk_meta": {
                        "page": page, "chunk": chunk_index,
                        "total_chunks": total_chunks_page
                    }
                }
                ...
                session.sql(
                    ...
                    params=[safe_str(api_id), api_target, response.status_code,
                            chunk_json, chunk_hash, annotated_url]
                ).collect()
    ...
    # ── MERGE staging → target (single atomic dedup operation) ──
    try:
    ...
    log_run_summary(session, run_id, api_target, api_id, run_start, pages_processed, total_chunks_all,
                    ...
                    last_sync_value or None,
                    safe_str(new_watermark) if watermark_persisted else None,
                    pagination_type)

    wm_msg = ""
    if incremental_flag:
        wm_msg = f" | Watermark: {last_sync_value or '(none)'} → {new_watermark or '(unchanged)'}"
    dup_msg = f" | {records_skipped:,} duplicate(s) skipped" if records_skipped else ""
    ...
    if bool(_cfg.get("EXPORT_ENABLED", False)):
        ...
        if has_export_config:
            ...
            export_msg = "pending — no export destination configured yet (set up in Data Explorer)"
    ...
    return f"Success: '{api_target}' — {pages_processed} page(s) · {records_ingested:,} records ingested{dup_msg}{wm_msg}{export_suffix}"
$$;
