...
def main(session, api_target):
    ...
    if incremental_flag and new_watermark is not None and str(new_watermark) != str(last_sync_value or ""):
        try:
            ...
            log_attempt(session, run_id, api_target, api_id, None, None, None,
                        f"Watermark persist failed: {str(wm_err)}", 0, None, True, None)
    ...
    return f"Success: '{api_target}' — {pages_processed} page(s) · {records_ingested:,} records ingested{dup_msg}{wm_msg}"
$$;
